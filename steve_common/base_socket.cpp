...
int CBaseSocket::SetNonBlockOption(int sock, bool flag /*= true*/)
{
	int save_mode;
	save_mode = fcntl( sock, F_GETFL, 0 );
	if (flag) { // set nonblock
		save_mode |= O_NONBLOCK;
	}
	else { // set block
...
int CBaseSocket::WriteN(unsigned int &nwrite, const void *pBuffer, unsigned int n)
{
	...
	while (nleft > 0)
	{
		if ( (ret = send(m_iSocket, ptr, nleft, 0)) < 0)
		{
			if (errno == EINTR)
...
int CBaseSocket::ReadN(unsigned int &nread,void *vptr, unsigned int n)
{
	...
	while (nleft > 0)
	{
		if ( (ret = recv(m_iSocket, ptr, nleft, 0)) < 0)
		{
			if (errno == EINTR)
				ret = 0;		/* and call read() again */
			else
...
int CBaseSocket::Connect(const char *pszHost, unsigned short nPort, int timeout_usec /*= -1*/)
{
	...
	/* 保存对端地址 */
	memcpy(&m_PeerAddr, &addr, sizeof(struct sockaddr_in));
